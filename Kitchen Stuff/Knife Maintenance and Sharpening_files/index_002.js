...
{Debug.write("Focussing");try{this.editor_window.focus();}catch(err){}},editor_set_controls:function()
...
this.wrap_tags('media',false,url);break;}
this.palettes[cmd].doClose();},emoticon_onclick:function(e)
{Event.stop(e);var elem=Event.element(e);var emo=elem.up('.emote');this.insert_emoticon(emo.readAttribute('emo_id'),emo.readAttribute('emo_img'),emo.readAttribute('emo_code'),e);},link_onclick:function(e)
...
{Debug.error(err);}}});;var _topic=window.IPBoard;_topic.prototype.topic={totalChecked:0,inSection:'',postcache:[],poll:[],pollPopups:[],init:function()
...
{pids.each(function(pid){if(!pid.blank()&&$('checkbox_'+pid))
{$('checkbox_'+pid).checked=true;ipb.topic.totalChecked++;}});}
ipb.topic.updatePostModButton();},checkPost:function(e,check)
{Debug.write("Check post");remove=$A();cookie=ipb.Cookie.get('modpids');if(cookie!=null){pids=cookie.split(',')||$A();}else{pids=$A();}
...
pids=pids.uniq().without(remove).join(',');ipb.Cookie.set('modpids',pids,0);ipb.topic.updatePostModButton();},updatePostModButton:function()
{if($('mod_submit'))
...
{elem=$('post_id_'+id).select('.post_ignore')[0];elem.hide();}
